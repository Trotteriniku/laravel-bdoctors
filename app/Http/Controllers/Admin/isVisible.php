...
return $this->sponsorships()->where('start_date', '<=', $now)->where('end_date', '>=', $now)->exists();
    ...
    @else
        <div> Acquista una sponsorizzazione per aumentare la tua visibilità.</div>
    @endif
    ...
    //1
    Endpoint API per la Visibilità: Crea un endpoint API nel tuo Laravel backend che restituisce lo stato di visibilità
    del dottore. Questo endpoint può semplicemente restituire il risultato del metodo isVisible() per il dottore
    autenticato.
    ...
    //3 Visualizzazione Condizionale in Vue: Usa la variabile isVisible per mostrare condizionalmente il messaggio di
    visibilità nel template del componente Vue.

    <div v-if="isVisible">Sei visibile sul sito.</div>
    <div v-else>Non sei attualmente visibile sul sito. Acquista una sponsorizzazione per aumentare la tua visibilità.
    </div>
